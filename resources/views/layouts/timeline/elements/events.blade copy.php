<div class="events">

    <div class="events-time">
        <div>
            <i class="events-increment events-up fa-solid fa-circle-up" title="Previous event"></i>
            <div>
                <i class="fa-regular fa-calendar"></i>
                <span>Timeline Start</span>                
            </div>
            <i class="events-increment events-down fa-solid fa-circle-down" title="Next event"></i>
        </div>
    </div>

    <div class="events-wrapper">

        <section class="year">

            <div itemscope itemtype="https://schema.org/Event" class="event-item event-first" data-order="-1">
                <!-- ISO 8601 date format) -->
                <h2 itemprop="startDate" content="2013-09-14T21:30" data-period="During 1998"><span class="event-moment">1998</span></h2>
                <div class="event">
                    <div class="event-wrapper">
                        <div>
                            <span class="fa-stack event-close">
                                <i class="fa-solid fa-circle fa-stack-2x"></i>
                                <i class="fa-solid fa-xmark fa-stack-1x"></i>
                            </span>
                            <div class="event-header">
                                <ul class="event-subheader">
                                    <li>
                                        Verified Time
                                    </li>
                                    <li>
                                        Verified Location
                                    </li>
                                </ul>
                                <div>
                                    <div>
                                        <h3 itemprop="name">
                                            Postumus's Gallic Empire is reabsorbed into the Roman Empire under Aurelian
                                        </h3>
                                        <div itemprop="location" itemscope itemtype="https://schema.org/Place">
                                            <p itemprop="address" itemscope itemtype="https://schema.org/PostalAddress">
                                                <i class="fas fa-map-marker-alt"></i><span itemprop="addressLocality">Missoula</span>, <span itemprop="addressRegion">MT</span>
                                            </p>
                                            <a itemprop="hasMap" itemtype="https://schema.org/Map" href="this url to this event loc on map">Map (hide)</a>
                                        </div>
                                        <span class="event-read">
                                            Read more<i class="fas fa-chevron-right"></i>
                                        </span>                                    
                                    </div>
                                    <img itemprop="image" src="https://images.pexels.com/photos/8971436/pexels-photo-8971436.jpeg" />
                                </div>
                            </div>
                            <div class="event-body">
                                <div itemprop="description">
                                    <p>
                                        Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                                    </p>
                                    <p>
                                        Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                                    </p>
                                </div>
                                <div class="event-sources">
                                    <h4>Sources</h4>
                                    <ul>
                                        <li>
                                            <i class="far fa-window-maximize"></i><a href="#" target="_blank">NY Post article</a>
                                        </li>
                                        <li>
                                            <i class="fab fa-youtube"></i><a href="#" target="_blank">Van is captured on video in very high quality</a><i class="fas fa-check event-verified"></i>
                                        </li>
                                        <li>
                                            <i class="fab fa-tiktok"></i><a href="#" target="_blank">TikTok videos uploaded</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="event-footer">
                            <ul class="event-resources">
                                <li class="event-source">
                                    <i class="far fa-file-alt"></i>3 sources
                                </li>
                                <li class="event-map">
                                    <i class="fas fa-map-marker-alt"></i>View <span>on</span> map
                                </li>
                            </ul>
                            <ul class="event-options">
                                <li class="event-comments" data-reveal="comments">
                                    <i class="fa-solid fa-comment"></i><span>2 <span>comments</span></span>
                                </li>
                                <li class="event-share">
                                    <i class="fa-solid fa-share-nodes"></i>
                                </li>
                                <li class="event-more dropdown-toggle">
                                    <i class="fa-solid fa-ellipsis dropdown-close"></i>
                                    <div class="dropdown" data-backdrop data-position="right">
                                        <ul>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-pencil"></i>Suggest an edit</a>
                                            </li>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-user-group"></i>Request to collaborate</a>
                                            </li>
                                            <span></span>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-circle-exclamation"></i>Report</a>
                                            </li>
                                        </ul>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>

        </section>

        <section class="none">

            <div class="event-item" data-order="0">
                <h2><span class="event-moment">Between 1999 and the start of 2021</span></h2>
                <div class="event">
                <div class="event-wrapper">
                        <div>
                            <span class="fa-stack event-close">
                                <i class="fa-solid fa-circle fa-stack-2x"></i>
                                <i class="fa-solid fa-xmark fa-stack-1x"></i>
                            </span>
                            <div class="event-header">
                                <ul class="event-subheader">
                                    <li>
                                        Verified Time
                                    </li>
                                    <li>
                                        Verified Location
                                    </li>
                                </ul>
                                <div>
                                    <div>
                                        <h3 itemprop="name">
                                            Postumus's Gallic Empire is reabsorbed into the Roman Empire under Aurelian
                                        </h3>
                                        <div itemprop="location" itemscope itemtype="https://schema.org/Place">
                                            <p itemprop="address" itemscope itemtype="https://schema.org/PostalAddress">
                                                <i class="fas fa-map-marker-alt"></i><span itemprop="addressLocality">Missoula</span>, <span itemprop="addressRegion">MT</span>
                                            </p>
                                            <a itemprop="hasMap" itemtype="https://schema.org/Map" href="this url to this event loc on map">Map (hide)</a>
                                        </div>
                                        <span class="event-read">
                                            Read more<i class="fas fa-chevron-right"></i>
                                        </span>                                    
                                    </div>
                                    <img itemprop="image" src="https://images.pexels.com/photos/8971436/pexels-photo-8971436.jpeg" />
                                </div>
                            </div>
                            <div class="event-body">
                                <div itemprop="description">
                                    <p>
                                        Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                                    </p>
                                    <p>
                                        Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                                    </p>
                                </div>
                                <div class="event-sources">
                                    <h4>Sources</h4>
                                    <ul>
                                        <li>
                                            <i class="far fa-window-maximize"></i><a href="#" target="_blank">NY Post article</a>
                                        </li>
                                        <li>
                                            <i class="fab fa-youtube"></i><a href="#" target="_blank">Van is captured on video in very high quality</a><i class="fas fa-check event-verified"></i>
                                        </li>
                                        <li>
                                            <i class="fab fa-tiktok"></i><a href="#" target="_blank">TikTok videos uploaded</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="event-footer">
                            <ul class="event-resources">
                                <li class="event-source">
                                    <i class="far fa-file-alt"></i>3 sources
                                </li>
                                <li class="event-map">
                                    <i class="fas fa-map-marker-alt"></i>View <span>on</span> map
                                </li>
                            </ul>
                            <ul class="event-options">
                                <li class="event-comments" data-reveal="comments">
                                    <i class="fa-solid fa-comment"></i><span>2 <span>comments</span></span>
                                </li>
                                <li class="event-share">
                                    <i class="fa-solid fa-share-nodes"></i>
                                </li>
                                <li class="event-more dropdown-toggle">
                                    <i class="fa-solid fa-ellipsis dropdown-close"></i>
                                    <div class="dropdown" data-backdrop data-position="right">
                                        <ul>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-pencil"></i>Suggest an edit</a>
                                            </li>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-user-group"></i>Request to collaborate</a>
                                            </li>
                                            <span></span>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-circle-exclamation"></i>Report</a>
                                            </li>
                                        </ul>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>

            <div class="event-item" data-order="1">
                <h2><span class="event-moment">Between 1999 and the start of 2021</span></h2>
                <div class="event">
                <div class="event-wrapper">
                        <div>
                            <span class="fa-stack event-close">
                                <i class="fa-solid fa-circle fa-stack-2x"></i>
                                <i class="fa-solid fa-xmark fa-stack-1x"></i>
                            </span>
                            <div class="event-header">
                                <ul class="event-subheader">
                                    <li>
                                        Verified Time
                                    </li>
                                    <li>
                                        Verified Location
                                    </li>
                                </ul>
                                <div>
                                    <div>
                                        <h3 itemprop="name">
                                            Postumus's Gallic Empire is reabsorbed into the Roman Empire under Aurelian
                                        </h3>
                                        <div itemprop="location" itemscope itemtype="https://schema.org/Place">
                                            <p itemprop="address" itemscope itemtype="https://schema.org/PostalAddress">
                                                <i class="fas fa-map-marker-alt"></i><span itemprop="addressLocality">Missoula</span>, <span itemprop="addressRegion">MT</span>
                                            </p>
                                            <a itemprop="hasMap" itemtype="https://schema.org/Map" href="this url to this event loc on map">Map (hide)</a>
                                        </div>
                                        <span class="event-read">
                                            Read more<i class="fas fa-chevron-right"></i>
                                        </span>                                    
                                    </div>
                                    <img itemprop="image" src="https://images.pexels.com/photos/8971436/pexels-photo-8971436.jpeg" />
                                </div>
                            </div>
                            <div class="event-body">
                                <div itemprop="description">
                                    <p>
                                        Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                                    </p>
                                    <p>
                                        Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                                    </p>
                                </div>
                                <div class="event-sources">
                                    <h4>Sources</h4>
                                    <ul>
                                        <li>
                                            <i class="far fa-window-maximize"></i><a href="#" target="_blank">NY Post article</a>
                                        </li>
                                        <li>
                                            <i class="fab fa-youtube"></i><a href="#" target="_blank">Van is captured on video in very high quality</a><i class="fas fa-check event-verified"></i>
                                        </li>
                                        <li>
                                            <i class="fab fa-tiktok"></i><a href="#" target="_blank">TikTok videos uploaded</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="event-footer">
                            <ul class="event-resources">
                                <li class="event-source">
                                    <i class="far fa-file-alt"></i>3 sources
                                </li>
                                <li class="event-map">
                                    <i class="fas fa-map-marker-alt"></i>View <span>on</span> map
                                </li>
                            </ul>
                            <ul class="event-options">
                                <li class="event-comments" data-reveal="comments">
                                    <i class="fa-solid fa-comment"></i><span>2 <span>comments</span></span>
                                </li>
                                <li class="event-share">
                                    <i class="fa-solid fa-share-nodes"></i>
                                </li>
                                <li class="event-more dropdown-toggle">
                                    <i class="fa-solid fa-ellipsis dropdown-close"></i>
                                    <div class="dropdown" data-backdrop data-position="right">
                                        <ul>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-pencil"></i>Suggest an edit</a>
                                            </li>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-user-group"></i>Request to collaborate</a>
                                            </li>
                                            <span></span>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-circle-exclamation"></i>Report</a>
                                            </li>
                                        </ul>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>

        </section>

        <section class="year">

            <div class="event-item event-start" data-order="2">
                <h2><span>During</span> <span class="event-moment">2021</span></h2>
                <div class="event">
                <div class="event-wrapper">
                        <div>
                            <span class="fa-stack event-close">
                                <i class="fa-solid fa-circle fa-stack-2x"></i>
                                <i class="fa-solid fa-xmark fa-stack-1x"></i>
                            </span>
                            <div class="event-header">
                                <ul class="event-subheader">
                                    <li>
                                        Verified Time
                                    </li>
                                    <li>
                                        Verified Location
                                    </li>
                                </ul>
                                <div>
                                    <div>
                                        <h3 itemprop="name">
                                            Postumus's Gallic Empire is reabsorbed into the Roman Empire under Aurelian
                                        </h3>
                                        <div itemprop="location" itemscope itemtype="https://schema.org/Place">
                                            <p itemprop="address" itemscope itemtype="https://schema.org/PostalAddress">
                                                <i class="fas fa-map-marker-alt"></i><span itemprop="addressLocality">Missoula</span>, <span itemprop="addressRegion">MT</span>
                                            </p>
                                            <a itemprop="hasMap" itemtype="https://schema.org/Map" href="this url to this event loc on map">Map (hide)</a>
                                        </div>
                                        <span class="event-read">
                                            Read more<i class="fas fa-chevron-right"></i>
                                        </span>                                    
                                    </div>
                                    <img itemprop="image" src="https://images.pexels.com/photos/8971436/pexels-photo-8971436.jpeg" />
                                </div>
                            </div>
                            <div class="event-body">
                                <div itemprop="description">
                                    <p>
                                        Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                                    </p>
                                    <p>
                                        Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                                    </p>
                                </div>
                                <div class="event-sources">
                                    <h4>Sources</h4>
                                    <ul>
                                        <li>
                                            <i class="far fa-window-maximize"></i><a href="#" target="_blank">NY Post article</a>
                                        </li>
                                        <li>
                                            <i class="fab fa-youtube"></i><a href="#" target="_blank">Van is captured on video in very high quality</a><i class="fas fa-check event-verified"></i>
                                        </li>
                                        <li>
                                            <i class="fab fa-tiktok"></i><a href="#" target="_blank">TikTok videos uploaded</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="event-footer">
                            <ul class="event-resources">
                                <li class="event-source">
                                    <i class="far fa-file-alt"></i>3 sources
                                </li>
                                <li class="event-map">
                                    <i class="fas fa-map-marker-alt"></i>View <span>on</span> map
                                </li>
                            </ul>
                            <ul class="event-options">
                                <li class="event-comments" data-reveal="comments">
                                    <i class="fa-solid fa-comment"></i><span>2 <span>comments</span></span>
                                </li>
                                <li class="event-share">
                                    <i class="fa-solid fa-share-nodes"></i>
                                </li>
                                <li class="event-more dropdown-toggle">
                                    <i class="fa-solid fa-ellipsis dropdown-close"></i>
                                    <div class="dropdown" data-backdrop data-position="right">
                                        <ul>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-pencil"></i>Suggest an edit</a>
                                            </li>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-user-group"></i>Request to collaborate</a>
                                            </li>
                                            <span></span>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-circle-exclamation"></i>Report</a>
                                            </li>
                                        </ul>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>

            <div class="event-item" data-order="3">
                <h2><span>During</span> <span class="event-moment">2021</span></h2>
                <div class="event">
                <div class="event-wrapper">
                        <div>
                            <span class="fa-stack event-close">
                                <i class="fa-solid fa-circle fa-stack-2x"></i>
                                <i class="fa-solid fa-xmark fa-stack-1x"></i>
                            </span>
                            <div class="event-header">
                                <ul class="event-subheader">
                                    <li>
                                        Verified Time
                                    </li>
                                    <li>
                                        Verified Location
                                    </li>
                                </ul>
                                <div>
                                    <div>
                                        <h3 itemprop="name">
                                            Postumus's Gallic Empire is reabsorbed into the Roman Empire under Aurelian
                                        </h3>
                                        <div itemprop="location" itemscope itemtype="https://schema.org/Place">
                                            <p itemprop="address" itemscope itemtype="https://schema.org/PostalAddress">
                                                <i class="fas fa-map-marker-alt"></i><span itemprop="addressLocality">Missoula</span>, <span itemprop="addressRegion">MT</span>
                                            </p>
                                            <a itemprop="hasMap" itemtype="https://schema.org/Map" href="this url to this event loc on map">Map (hide)</a>
                                        </div>
                                        <span class="event-read">
                                            Read more<i class="fas fa-chevron-right"></i>
                                        </span>                                    
                                    </div>
                                    <img itemprop="image" src="https://images.pexels.com/photos/8971436/pexels-photo-8971436.jpeg" />
                                </div>
                            </div>
                            <div class="event-body">
                                <div itemprop="description">
                                    <p>
                                        Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                                    </p>
                                    <p>
                                        Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                                    </p>
                                </div>
                                <div class="event-sources">
                                    <h4>Sources</h4>
                                    <ul>
                                        <li>
                                            <i class="far fa-window-maximize"></i><a href="#" target="_blank">NY Post article</a>
                                        </li>
                                        <li>
                                            <i class="fab fa-youtube"></i><a href="#" target="_blank">Van is captured on video in very high quality</a><i class="fas fa-check event-verified"></i>
                                        </li>
                                        <li>
                                            <i class="fab fa-tiktok"></i><a href="#" target="_blank">TikTok videos uploaded</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="event-footer">
                            <ul class="event-resources">
                                <li class="event-source">
                                    <i class="far fa-file-alt"></i>3 sources
                                </li>
                                <li class="event-map">
                                    <i class="fas fa-map-marker-alt"></i>View <span>on</span> map
                                </li>
                            </ul>
                            <ul class="event-options">
                                <li class="event-comments" data-reveal="comments">
                                    <i class="fa-solid fa-comment"></i><span>2 <span>comments</span></span>
                                </li>
                                <li class="event-share">
                                    <i class="fa-solid fa-share-nodes"></i>
                                </li>
                                <li class="event-more dropdown-toggle">
                                    <i class="fa-solid fa-ellipsis dropdown-close"></i>
                                    <div class="dropdown" data-backdrop data-position="right">
                                        <ul>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-pencil"></i>Suggest an edit</a>
                                            </li>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-user-group"></i>Request to collaborate</a>
                                            </li>
                                            <span></span>
                                            <li>
                                                <a href="#"><i class="fa-solid fa-circle-exclamation"></i>Report</a>
                                            </li>
                                        </ul>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>

            <section class="month">

                <div class="event-item" data-order="4">
                    <h2><span>During</span> <span class="event-moment">September</span> <span>2021</span></h2>
                    <div class="event">
                        e
                    </div>
                </div>

                <section class="day">

                    <h2><span>On</span> <span class="event-moment"><em>Wed<span>nesday</span></em> 15<sup>th</sup></span> <span>September</span> <span>2021</span></h2>

                    <section class="time">

                        <div class="event-item" data-order="5">
                            <h2><span>At 5:15pm on <em>Wed<span>nesday</span></em> 15<sup>th</sup> September 2021</span></h2>
                            <div class="event">
                                e
                            </div>
                        </div>

                        <div class="event-item" data-order="6">
                            <h2><span>At 9:15pm on <em>Wed<span>nesday</span></em> 15<sup>th</sup> September 2021</span></h2>
                            <div class="event">
                                9:15pm (4 hours later)
                            </div>
                        </div>

                        <div class="event-item" data-order="7">
                            <h2><span>After 9:15pm on <em>Wed<span>nesday</span></em> 15<sup>th</sup> September 2021</span></h2>
                            <div class="event">
                                event
                            </div>
                        </div>
                        
                        <div class="event-item" data-order="8">
                            <h2><span>After 9:15pm on <em>Wed<span>nesday</span></em> 15<sup>th</sup> September 2021</span></h2>
                            <div class="event">
                                event
                            </div>
                        </div>

                    </section>

                </section>

                <section class="day">

                    <div class="event-item" data-order="9">
                        <h2><span>On</span> <span class="event-moment"><em>Thu<span>rsday</span></em> 16<sup>th</sup></span> <span>September</span> <span>2021</span></h2>
                        <div class="event">
                            event
                        </div>
                    </div>

                </section>

                <div class="event-item" data-order="10">
                    <h2><span>During</span> <span class="event-moment">September</span> <span>2021</span></h2>
                    <div class="event">
                        event
                    </div>
                </div>

            </section>

            <section class="month">

                <div class="event-item" data-order="11">
                    <h2><span>During</span> <span class="event-moment">October</span> <span>2021</span></h2>
                    <div class="event">
                        event
                    </div>
                </div>

            </section>

        </section>

        <section class="none">

            <div class="event-item" data-order="12">
                <h2><span>Between <em>Oct<span>ober</span></em> 2021 and the start of 2022</span></h2>
                <div class="event">
                    event
                </div>
            </div>

            <div class="event-item" data-order="13">
                <h2><span>Between <em>Oct<span>ober</span></em> 2021 and the start of 2022</span></h2>
                <div class="event">
                    event
                </div>
            </div>

        </section>

        <section class="year">

            <div class="event-item event-last" data-order="14">
                <h2><span>During</span> <span class="event-moment">2022</span></h2>
                <div class="event">
                    event
                </div>
            </div>

        </section>



        <!--<section data-value="Start">

            <h3>
                <span>Start</span>
            </h3>

            <div class="event">simple event with no time etc</div>

        </section>

        <section data-value="2021">

            <h3>
                <span class="year">2021</span>
            </h3>

            <div class="event">simple event with no time etc</div>

        </section>

            <section class="month" data-period="month" data-value="September 2021">

                <h3>
                    <span class="month">September</span>
                    <span class="year">2021</span>
                </h3>

                <section data-period="">

                    <div class="event">simple event with no time etc</div>

                </section>

                <section class="day" data-period="day">

                    <h3>
                        <span class="day">Wed<span>nesday</span> 15<sup>th</sup></span>
                        <span class="month">September</span>
                        <span class="year">2021</span>
                    </h3>
                    <div class="event">something happened</div>

                    <section class="time" data-period="time">

                        <h3>
                            <span class="day">Wed<span>nesday</span> 15<sup>th</sup></span>
                            <span class="month">September</span>
                            <span class="year">2021</span>
                            <span class="time">5:15pm</span>
                        </h3>
                        <div class="event">something happened</div>

                    </section>

                    <section data-period="">

                        <div class="event">simple event with no time etc</div>

                    </section>

                    <section class="time" data-period="time">

                        <h3>
                            <span class="day">Wed<span>nesday</span> 15<sup>th</sup></span>
                            <span class="month">September</span>
                            <span class="year">2021</span>
                            <span class="time">8:01pm</span>
                        </h3>
                        <div class="event">something happened</div>

                    </section>

                </section>

                <section class="day" data-period="day">

                    <h3>
                        <span class="day">Thu<span>rsday</span> 16<sup>th</sup></span>
                        <span class="month">September</span>
                        <span class="year">2021</span>
                    </h3>
                    <div class="event">something happened</div>

                </section>

            </section>

            <section class="month" data-period="month">

                <h3>
                    <span class="month">October</span>
                    <span class="year">2021</span>
                </h3>

                <section class="day" data-period="day">

                    <h3>
                        <span class="day">Monday 19<sup>th</sup></span>
                        <span class="month">October</span>
                        <span class="year">2021</span>
                    </h3>

                    <section class="time" data-period="time">

                        <h3>
                            <span class="day">Monday 19<sup>th</sup></span>
                            <span class="month">October</span>
                            <span class="year">2021</span>
                            <span class="time">8:19pm</span>
                        </h3>
                        <div class="event">something happened</div>
                        
                    </section>

                </section>

            </section>

        </section>

        <section class="year" data-period="year">

            <h3>2022</h3>

            <section class="month month__september" data-period="month">

                <h3>Sept</h3>

                <section class="day" data-period="day">

                    <h3>Fri 15<sup>th</sup></h3>
                    <div class="event">something happened</div>

                    <section class="time" data-period="time">

                        <h3>5:15am</h3>
                        <div class="event">something happened</div>

                    </section>

                    <section data-period="">

                        <div class="event">simple event with no time etc</div>

                    </section>

                    <section class="time" data-period="time">

                        <h3>4:15pm</h3>
                        <div class="event">something happened</div>

                    </section>

                </section>

                <section class="day" data-period="day">

                    <h3>Sat 16<sup>th</sup></h3>
                    <div class="event">something happened</div>

                </section>

            </section>

            <section class="month month__october" data-period="month">

                <h3>Oct</h3>

                <section class="day" data-period="day">

                    <h3>Tue 19<sup>th</sup></h3>

                    <section class="time" data-period="time">

                        <h3>8:19pm</h3>
                        <div class="event">something happened</div>
                        
                    </section>

                </section>

            </section>

        </section>-->




            <!---<div class="dots"><div></div><div></div><div></div><div></div></div>

                <li class="dropdown-toggle">
                    <i class="fa-solid fa-plus"></i>normal
                    <div class="dropdown">
                        <ul>
                            <li>
                                <input type="checkbox">
                            </li>
                            <li>
                                <a href="https://google.com"><i class="fa-regular fa-user"></i>thanks</a>
                            </li>
                            <li>
                                <a href="#"><i class="fa-solid fa-pencil"></i>Suggest an edit</a>
                            </li>
                            <span></span>
                            <li>
                                <a href="#"><i class="fa-solid fa-circle-exclamation"></i>Report</a>
                            </li>
                        </ul>
                    </div>
                </li>
                <br/><br/><br/><br/><br/>

                <li class=" dropdown-toggle">
                    <i class="fa-solid fa-plus"></i>normal remain
                    <div class="dropdown">
                        <ul>
                            <li>
                                <input type="checkbox">
                            </li>
                            <li>
                                <a href="#"><i class="fa-regular fa-user"></i>no remain</a>
                            </li>
                            <li>
                                <a href="#"><i class="fa-solid fa-pencil"></i>Suggest an edit</a>
                            </li>
                            <span></span>
                            <li>
                                <a href="#"><i class="fa-solid fa-circle-exclamation"></i>DONE</a>
                            </li>
                        </ul>
                    </div>
                </li>

                <br/><br/><br/><br/><br/>

                <li class=" dropdown-toggle">
                    <i class="fa-solid fa-plus"></i>backdrop
                    <div class="dropdown" data-backdrop>
                        <ul>
                            <li>
                                <input type="checkbox">
                            </li>
                            <li>
                                <a href="#"><i class="fa-regular fa-user"></i>bg plz</a>
                            </li>
                            <li>
                                <a href="#"><i class="fa-solid fa-pencil"></i>Suggest an edit</a>
                            </li>
                            <span></span>
                            <li>
                                <a href="#"><i class="fa-solid fa-circle-exclamation"></i>Report</a>
                            </li>
                        </ul>
                    </div>
                </li>

                <br/><br/><br/><br/><br/>

                <li class=" dropdown-toggle">
                    <i class="fa-solid fa-plus"></i>backdrop remain
                    <div class="dropdown" data-backdrop>
                        <ul>
                            <li>
                                <input type="checkbox">
                            </li>
                            <li>
                                <a href="#"><i class="fa-regular fa-user"></i>bg plz1</a>
                            </li>
                            <li>
                                <a href="#"><i class="fa-solid fa-pencil"></i>Suggest an edit</a>
                            </li>
                            <span></span>
                            <li>
                                <a class="dropdown-action" href="#"><i class="fa-solid fa-circle-exclamation"></i>DONE</a>
                            </li>
                        </ul>
                    </div>
                </li>--->
    </div>

</div>