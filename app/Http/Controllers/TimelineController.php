    ...
    public function events(Timeline $timeline, Request $request) 
    {
        ...
        if ($request->ajax()){
            ...
            if ($timeline_events->count()) {
                ...
                $events_markers = $timeline_events->where('location_show', 1)->map->only([ 'id', 'title',  'period',  'location', 'location_lat', 'location_lng', 'location_zoom', 'order_overall' ])->sortBy('order_overall')->values()->all();
